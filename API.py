...
class App_CE(tk.Tk):
    ...
    def generate_graph2(self, abs):
        ...
        df['TauxOccupation'].plot(ax=ax)

        ax.scatter([abs, ], [df.at[abs, "TauxOccupation"], ], 50, color='red')
    ...
    def get_val(self, i, seuil_max = 40):
        df = pd.read_csv(self.file_path)
        occupancy_rate = df.at[i, "TauxOccupation"]
        date = df.at[i, "Datetime"]
        max_occupancy = df["TauxOccupation"].max()
        min_occupancy = df["TauxOccupation"].min()
...
class App_convention(tk.Tk):
    def __init__(self, file_path = "result/submission.csv"):
        super().__init__()
        self.file_path = file_path
        self.title("Afficher les résultats")
